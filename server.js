...
function updatePlayerAI(player) {
    ...
    if (player.role === 'GK') {
        ...
        if (opponentHasPossession && distSq(playerOwner.x, playerOwner.y, ownGoalX, goalY) < threatDistSq) { targetY = FIELD_HEIGHT / 2 + (ball.y - FIELD_HEIGHT / 2) * 0.6; targetX = player.baseX + (player.team === 'A' ? -15 : 15); }
        else if (ballIsLoose && distSq(ball.x, ball.y, player.baseX, player.baseY) < (FIELD_WIDTH*0.2)**2 ) { targetX = ball.x; targetY = ball.y; }
        else { targetY = player.baseY + (ball.y - player.baseY) * 0.15; targetX = player.baseX; }
        const penaltyAreaWidth = 165; const goalLine = (player.team === 'A' ? 0 : FIELD_WIDTH); const nearPost = (FIELD_HEIGHT - GOAL_WIDTH)/2 - PLAYER_RADIUS; const farPost = (FIELD_HEIGHT + GOAL_WIDTH)/2 + PLAYER_RADIUS;
        targetX = player.team === 'A' ? Math.max(goalLine, Math.min(goalLine + penaltyAreaWidth, targetX)) : Math.max(goalLine - penaltyAreaWidth, Math.min(goalLine, targetX));
        targetY = Math.max(nearPost, Math.min(farPost, targetY));
        player.targetX = targetX; player.targetY = targetY; movePlayerTowardsTarget(player, PLAYER_SPEED * 0.8);
        if (ballIsLoose && playerDistToBallSq < controlRangeSq * 1.5 && player.kickCooldown <= 0) { gainPossession(player); const clearTargetX = FIELD_WIDTH / 2; const clearTargetY = player.y < FIELD_HEIGHT / 2 ? FIELD_HEIGHT * 0.2 : FIELD_HEIGHT * 0.8; shootBall(player, clearTargetX, clearTargetY, SHOT_POWER * 0.8); player.kickCooldown = 40; }
        return;
    }
    ...
    if (teamHasPossession) {
        if (playerOwner === player) { // A.1) This player has the ball
            player.state = 'ATTACKING_WITH_BALL'; ball.ownerId = player.id;
            const distToGoalSq = distSq(player.x, player.y, goalX, goalY); const shootRangeSq = (FIELD_WIDTH * SHOOTING_RANGE_FACTOR)**2; const { opponent: nearestOpponent, distSq: oppDistSq } = findClosestOpponent(player); const underPressure = nearestOpponent && oppDistSq < (PLAYER_RADIUS * 5)**2;
            if (distToGoalSq < shootRangeSq && player.kickCooldown <= 0 && Math.random() < SHOT_PROBABILITY) { const shotAngleGood = Math.abs(player.y - goalY) < GOAL_WIDTH * 1.5; if (shotAngleGood || Math.random() < 0.1) { player.state = 'SHOOTING'; shootBall(player, goalX, goalY + (Math.random() - 0.5) * GOAL_WIDTH * 0.8); stats[player.team === 'A' ? 'teamA' : 'teamB'].shots++; player.kickCooldown = 25; return; } }
             if (!underPressure && player.kickCooldown <= 0 && Math.random() < PASS_PROBABILITY) { let bestPassTarget = null; let bestScore = -Infinity; players.forEach(p => { if (p.team === player.team && p !== player) { const targetDistToGoalSq = distSq(p.x, p.y, goalX, goalY); let score = (distToGoalSq - targetDistToGoalSq) - distSq(player.x, player.y, p.x, p.y) * 0.1; if ((player.team === 'A' && p.x < player.x - 50) || (player.team === 'B' && p.x > player.x + 50)) { score -= 5000; } if (score > bestScore) { bestScore = score; bestPassTarget = p; } } }); if (bestPassTarget && bestScore > -1000) { player.state = 'PASSING'; passBall(player, bestPassTarget); stats[player.team === 'A' ? 'teamA' : 'teamB'].passes++; player.kickCooldown = 15; return; } }
            player.state = 'DRIBBLING'; let dribbleTargetX = goalX; let dribbleTargetY = goalY; if (underPressure && nearestOpponent) { const angleToOpponent = Math.atan2(nearestOpponent.y - player.y, nearestOpponent.x - player.x); const evadeAngle = angleToOpponent + (player.y < goalY ? -Math.PI / 2 : Math.PI / 2); dribbleTargetX = player.x + Math.cos(evadeAngle) * 50; dribbleTargetY = player.y + Math.sin(evadeAngle) * 50; dribbleTargetX = (dribbleTargetX + goalX) / 2; dribbleTargetY = (dribbleTargetY + goalY) / 2; }
            player.targetX = player.x + (dribbleTargetX - player.x) * 0.1; player.targetY = player.y + (dribbleTargetY - player.y) * 0.1; movePlayerTowardsTarget(player, PLAYER_SPEED * 0.9); const angleToTarget = Math.atan2(player.targetY - player.y, player.targetX - player.x); ball.x = player.x + Math.cos(angleToTarget) * (PLAYER_RADIUS + BALL_RADIUS * 0.8); ball.y = player.y + Math.sin(angleToTarget) * (PLAYER_RADIUS + BALL_RADIUS * 0.8); ball.vx = player.vx; ball.vy = player.vy;
            return;
        } else { // A.2) Teammate has the ball - Support
            player.state = 'ATTACKING_SUPPORT'; const angleToGoal = Math.atan2(goalY - playerOwner.y, goalX - playerOwner.x); const supportDist = SUPPORT_DISTANCE_AVG + (Math.random() - 0.5) * 40; let baseTargetX = playerOwner.x + Math.cos(angleToGoal) * supportDist; let baseTargetY = playerOwner.y + Math.sin(angleToGoal) * supportDist;
            if (player.role === 'FWD' || (player.role === 'MID' && Math.random() < 0.5)) { const sideOffset = (player.baseY < FIELD_HEIGHT / 2 ? -1 : 1) * (50 + Math.random() * 50); baseTargetY += sideOffset; } else if (player.role === 'DEF') { baseTargetX = (player.baseX + playerOwner.x) / 2; baseTargetY = (player.baseY + playerOwner.y) / 2; }
            player.targetX = (baseTargetX + player.baseX) / 2; player.targetY = (baseTargetY + player.baseY) / 2; player.targetX = Math.max(PLAYER_RADIUS, Math.min(FIELD_WIDTH - PLAYER_RADIUS, player.targetX)); player.targetY = Math.max(PLAYER_RADIUS, Math.min(FIELD_HEIGHT - PLAYER_RADIUS, player.targetY)); movePlayerTowardsTarget(player);
            return;
        }
    }
    else if (opponentHasPossession) { // **B) Opponent Has Possession**
        const chaseRangeFactor = player.role === 'FWD' ? CHASE_RANGE_FACTOR_FWD : (player.role === 'MID' ? CHASE_RANGE_FACTOR_MID : CHASE_RANGE_FACTOR_DEF); const chaseRangeSq = (FIELD_WIDTH * chaseRangeFactor) ** 2; const distToOwnerSq = distSq(player.x, player.y, playerOwner.x, playerOwner.y);
        if (distToOwnerSq < chaseRangeSq * 0.5) { // B.1) Close enough to press
            player.state = 'DEFENDING_PRESS'; player.targetX = playerOwner.x; player.targetY = playerOwner.y; movePlayerTowardsTarget(player, PLAYER_SPEED * 1.05);
             if (distToOwnerSq < controlRangeSq && player.kickCooldown <= 0 && Math.random() < TACKLE_PROBABILITY) { logDebug(`${player.id} attempts tackle on ${playerOwner.id}`); ball.ownerId = null; ball.vx = (Math.random() - 0.5) * 5; ball.vy = (Math.random() - 0.5) * 5; player.kickCooldown = TACKLE_COOLDOWN; playerOwner.kickCooldown = DISPOSSESSED_COOLDOWN; }
            return;
        } else { // B.2) Not pressing - Mark or return
            player.state = 'DEFENDING_POSITION'; let closestOppToMark = null; let minDistSq = Infinity; players.forEach(p => { if (p.team !== player.team && p !== playerOwner) { const dSq = distSq(player.x, player.y, p.x, p.y); if (dSq < (FIELD_WIDTH * 0.3)**2 && dSq < minDistSq) { minDistSq = dSq; closestOppToMark = p; } } });
             if (closestOppToMark && player.role !== 'FWD') { player.targetX = (closestOppToMark.x + ownGoalX) / 2; player.targetY = (closestOppToMark.y + goalY) / 2; player.targetX = (player.targetX + player.baseX) / 2; player.targetY = (player.targetY + player.baseY) / 2; }
             else { player.targetX = player.baseX; player.targetY = player.baseY; const ballDistToOwnGoalFactor = Math.max(0, 1 - distSq(ball.x, ball.y, ownGoalX, goalY) / (FIELD_WIDTH*FIELD_WIDTH) ); const defensiveShift = (player.baseX - ownGoalX) * 0.3 * ballDistToOwnGoalFactor; player.targetX -= defensiveShift * (player.team === 'A' ? 1 : -1); }
            player.targetX = Math.max(PLAYER_RADIUS, Math.min(FIELD_WIDTH - PLAYER_RADIUS, player.targetX)); player.targetY = Math.max(PLAYER_RADIUS, Math.min(FIELD_HEIGHT - PLAYER_RADIUS, player.targetY)); movePlayerTowardsTarget(player);
            return;
        }
    }
    else if (ballIsLoose) { // **C) Ball is Loose**
         const chaseRangeFactor = player.role === 'FWD' ? CHASE_RANGE_FACTOR_FWD * 1.1 : (player.role === 'MID' ? CHASE_RANGE_FACTOR_MID * 1.05 : CHASE_RANGE_FACTOR_DEF); const chaseRangeSq = (FIELD_WIDTH * chaseRangeFactor) ** 2;
        if (playerDistToBallSq < chaseRangeSq) { // C.1) Close enough to chase
            player.state = 'CHASING_LOOSE_BALL'; player.targetX = ball.x; player.targetY = ball.y; movePlayerTowardsTarget(player, PLAYER_SPEED * 1.1);
            if (playerDistToBallSq < controlRangeSq && player.kickCooldown <= 0) { gainPossession(player); }
            return;
        } else { // C.2) Ball is loose but too far away
             player.state = 'RETURNING_TO_POSITION'; player.targetX = player.baseX; player.targetY = player.baseY; movePlayerTowardsTarget(player);
             return;
        }
    }

    // Fallback
    player.state = 'IDLE_RETURN'; player.targetX = player.baseX; player.targetY = player.baseY; movePlayerTowardsTarget(player);
}
...
function createFullGameStatePayload() {
    return {
        gameState, scoreA, scoreB, teamA, teamB, oddsA, oddsB,
        serverGameTime: calculateCurrentDisplayTime(), // Now defined below
